...
// prerequisite:true — 이건 "직전 스테이지가 산출물을 남겼는가"를 묻는 선행 확인이지 이번 런의
// 게이트 판정이 아니다(게이트도 sha 바인딩도 아직 존재하지 않는다).
const req = target ? requirementFor(target)({ comments: await gh.comments(issue), issue, prerequisite: true }) : { ok: true };
