...
stateDiagram-v2
  ...
  needs_info --> queue: 사람 · sweeper 주차 해제
  ...
  blocked --> queue: blocked 재시도 (origin=queue)
  blocked --> ready: blocked 재시도 (origin=ready)
  blocked --> planned: blocked 재시도 (origin=planned|in_progress)
  blocked --> awaiting_review: blocked 재시도 (origin=awaiting_review)
  blocked --> approved: blocked 재시도 (origin=approved, 게이트 재확인 뒤)
  blocked --> rework: merge 재시도에서 PR CONFLICTING
  in_progress --> needs_info: 하네스 대기 주차
  awaiting_review --> needs_human: 산출물 검증 실패 · round ≥ K
  needs_human --> queue: 사람
  needs_human --> ready: 사람 재시도 (중단 지점=ready)
...
파일과 sha 바인딩만** 면제된다(`blocked` hop-back의 `prerequisite`와 같은 이유다 — 그 판정은 되돌아간
스테이지가 다시 돌며 만든다). 이 전이는 리뷰 라운드로 세지 않는다(`reason=retry` — 재큐가 아니다).
목적지는 `:unstick`의 `retry` 결정이 고르고, 근거는 같은 이슈의 `human-decision:v1` 코멘트다(§13.3).

`blocked --> …` 재시도 엣지들은 sweeper의 blocked 팔과 `run-stage`의 진입 가드가 **`factory-blocked-origin` 마커 하나로** 함께 판정한다(§4.3-2, ADR-020 KTB-15b·22·24): 그 blocked이 **그 스테이지 자신의 정상 진입 라벨에서** 왔을 때만 되돌아간다(그 외에는 전이 없이 거부하고 사람에게 간다). 되돌아가는 자리는 언제나 그 스테이지의 진입 라벨이고(`in_progress`에서 온 blocked도 `planned`로 간다 — implement가 그 자리를 다시 채운다), merge만 예외로 게이트를 이번 런에서 다시 GREEN으로 확인한 **뒤에** `approved`로 돌아간다.

**전이의 원자성과 복구**(ADR-020 KTB-30): 라벨 교체는 **새 상태를 먼저 붙이고 옛 상태를 나중에 떼는** 두 번의 호출이며(각각 1s·3s·9s 재시도 + REST 폴백, 쓴 뒤 확인 1회), 전이 코멘트는 그 교체보다 **먼저** 나간다 — 요구사항 미달 거부(`* → needs_human`)도 같다(마커 `factory-transition:v1 … reason=refused`, ADR-020 r2). 교체가 통째로 실패하면 그 자리에 `factory-transition-failed:v1` 한 줄이 남아 앞의 전이 코멘트를 무효로 만든다(라운드 카운터도 그 짝을 빼고 센다). 그 사이에서 실패해 상태 라벨이 2개가 되거나 0개가 된 이슈는 sweeper의 라벨-셋 복구 팔(§4.3-7)과 상태-라벨 복구 팔(§4.3-8)이 **최신 전이 코멘트의 `to`** 하나로 되돌린다 — 상태 라벨이 0개인 이슈는 이벤트·sweeper·`factory status` 어디에도 나타나지 않기 때문이다.
...
1. `Aborted cleanup` — `if: always() && job.status != 'success'`, `run-stage.js <stage> <issue> --aborted "${{ job.status }}"`.
   (`cancelled() || failure()`가 아닌 이유: 그 둘은 "런이 취소됐다"와 "앞 스텝이 실패했다"만 참으로 만들어
   잡 타임아웃·러너 소실을 덮지 못한다 — ADR-020 KTB-24 r1. 이 스텝의 `FACTORY_RUNNER_ID`는 `Run stage`의
   것과 **같은 식**이어야 한다: 정리가 그 값으로 "이 락이 내 것인가"를 가른다(린트 `runner-id-consistent`).)
...
run-stage.js <stage> <issue>
  ...
  5. gates.js <level>                        # implement/review/merge. 에이전트 밖에서 실행. 판정 파일 .factory/out/gates.json 생성
                                             ...
                                             #   깨진 테스트가 0개인 RED(ADR-020 KTB-35) — 명령은 exit≠0인데 **읽어낸** 리포트의 실패가 0개면
                                             #   판정은 RED 그대로지만(fail closed) 그 게이트가 `reason: "command exited <code> with 0 failing
                                             #   tests — unhandled error outside tests"`와 stderr 마지막 20줄(스크럽됨)을 함께 싣는다. 스테이지는
                                             #   그 사유로 `factory:blocked`(cause `gates-unhandled`)이 되고 sweeper가 같은 스테이지를 한 번 다시
                                             #   돌린다 — 테스트 밖에서 죽은 런은 대개 일시적 인프라다(포크된 워커의 stderr EPIPE).
...
- **sweeper**(`factory-sweeper.yml`, 30분 주기 → `.factory/bin/sweep.js`)가 **여덟 가지**를 훑는다. 1·2·5·6·7·8이 **상태 복구 팔**(`--quick`이 도는 것), 3·4가 **시간에 묶인 팔**(cron의 몫)이다. 실행 순서는 번호순이 아니라 **7·8을 맨 앞에** 둔다(ADR-020 r2): 라벨 변경 하나가 최대 13초를 자게 된 뒤로 잡 시간은 유한한 자원이고, 넓은 API 장애 — 곧 두 복구 팔이 가장 할 일이 많은 상황 — 에서 앞선 팔들이 예산을 다 쓰면 장애를 치우려고 만든 팔이 장애 때 돌지 않는다. 두 팔은 다른 팔의 **입력**(상태 라벨)을 고치므로 앞에 두는 편이 같은 sweep 안에서 더 정확하기도 하다:
  1. `factory:in-progress` 이슈의 heartbeat 코멘트(`<!-- factory-heartbeat issue=<n> -->`)가 30분 넘게 갱신되지 않았으면 lock을 회수하고(**아래 5와 같은 리스 삭제**다 — 하트비트는 best-effort로 패치되므로 "늦었다"가 "죽었다"의 증명이 아니다; 소유자가 **살아 있다고 확인되면 재큐하지 않는다** — 그 재큐가 R 예산을 태운다, ADR-020 r2; 소유자가 **`unknown`이면**(로컬 러너 `runner=local/<host>`, 또는 Actions 조회 실패) 아래 5·2와 **똑같이** 재큐하지 않고 임계를 넘겼을 때 `lock owner unknowable (<detail>)`로 `factory:needs-human`에 올린다 — ADR-020 최종 리뷰 MF-4: 살아 있는 로컬 런을 재큐하면 R 예산이 타고, 그 런이 끝내 GREEN을 밀어도 라벨이 이미 `planned`라 `in-progress → awaiting-review`가 그래프에서 거부되어 완성된 구현이 좌초한다), 같은 이슈의 `factory-retry issue=<n> count=<k>` 마커를 읽어 count+1이 R 이하면 `factory:planned`로 되돌린다(재큐 — §3.2 `in_progress --> planned` 엣지, 다음 재큐 코멘트에 갱신된 count가 남는다). count가 R을 넘으면 `factory:needs-human`으로 보낸다.
  2. `factory:blocked` 이슈는 기본적으로 `factory:needs-human`으로 올린다 — 환경·크리덴셜 문제는 sweeper가 고칠 수 없다(§3.2 `blocked --> needs_human`). **예외**(KTB-15b/22/24): `factory-blocked-origin` 마커가 말하는 origin이 그 스테이지 자신의 정상 진입 라벨이면 그 스테이지를 **한 번** 다시 띄운다(§3.2 `blocked --> queue|ready|planned|awaiting_review|approved|rework`). 사유가 API 쿼터/장애면 3회까지다. 원인 등급 `gates-unhandled`(테스트 명령이 exit≠0인데 깨진 테스트는 0개 — ADR-020 KTB-35)도 같은 한 번짜리 재시도를 받고, 에스컬레이션 문구가 그 원인을 이름으로 말한다. 밀기 직전에 잔해 락을 회수한다(아래 5와 같은 판정·같은 리스, KTB-28 — 락이 `live`면 이 재시도도 시도 번호를 쓰지 않고 다음 sweep으로 미루고, `unknown`이면서 스톨 임계를 넘겼으면 `lock owner unknowable`로 사람에게 올린다). origin 마커의 `cause=` 등급(ADR-020 O20)이 두 가지를 가른다: 사람이 취소한 blocked(`cancelled`)은 R 예산을 쓰지 않고 **취소 사건마다** 한 번 다시 밀리며(상한 3회), 에스컬레이션 사유는 등급을 이름으로 말한다(`blocked (job cancelled) — needs human` 등).
...
- **sweeping은 cron만이 아니다(ADR-020 KTB-26).** `schedule`은 best-effort라 실측에서 몇 시간짜리 공백이
  ...
  `sweep.js --quick`을 돈다(§4.1). `--quick`은 위 1·2와 멈춘 스테이지 재점화·하네스 주차 해제·라벨-셋 복구·상태 라벨 0개 복구 — 곧 **상태
  복구 팔만** 돌고(cron과 **같은 순서**다 — 두 복구 팔이 맨 앞), 시간에 묶인 3·4(격리 정책, 토큰 만료)는 cron의 몫으로 남는다. 둘은 대체가 아니라
...
- **토큰은 둘이고, 배우도 둘이다(ADR-021).** `FACTORY_BOT_TOKEN` = **에이전트 배우** — admin이 아닌 계정(write 협력자인 머신 유저, 나중에는 GitHub App)의 PAT이고, 체크아웃·코멘트·라벨·락 브랜치·PR 생성·`factory/*` push가 전부 이것으로 나간다. `claude -p`가 도는 스테이지가 보는 유일한 자격증명이다. `FACTORY_MERGE_TOKEN` = **머지 배우** — admin/owner PAT이고 **스크립트 전용 잡에만** 등장한다(머지 스테이지의 `Run stage` 스텝 + 그 잡의 자격증명 스크럽 스텝). 값은 저장소 시크릿이 아니라 **`factory-merge` 환경 시크릿**이다(ADR-021 r1 MF-2 b — 저장소 시크릿은 같은 저장소의 어느 브랜치에서 돌든 워크플로에 주어진다). 에이전트 PAT의 스코프는 **`repo` 하나**이고 `workflow`를 포함하지 않는다(r1 MF-2 a). 에이전트 스테이지의 `checkout` `token:`에도, 그 스텝들의 env에도 들어가지 않으며 sweeper도 받지 않는다(락 브랜치는 보호되지 않으므로 write면 충분하다). `yml-lint`의 `merge-token-scope` 규칙이 그 범위를 강제한다.
  - **두 배우 모드**(`FACTORY_MERGE_TOKEN`이 있음): `bootstrap`이 base 브랜치에 `required_pull_request_reviews = { required_approving_review_count: 1, dismiss_stale_reviews: true, require_code_owner_reviews: true }`를 걸고, `.github/CODEOWNERS`를 `* @<머지 배우 로그인>`으로 쓰고, `factory-merge` 환경(기본 브랜치 배포만)을 만든다. 그래서 에이전트 배우가 쥔 토큰으로는 **어떤 PR도** 머지할 수 없다 — 자기가 연 PR은 GitHub이 자기-승인을 거부해서(422), 남이 연 PR은 그 한 건의 승인이 **코드 오너(=머지 배우)**의 것이어야 해서. 수만 세는 규칙으로는 후자가 열려 있었다(ADR-021 r1 MF-1: write 협력자는 자기가 열지 않은 PR의 head에 커밋을 push한 뒤 스스로 approve할 수 있다). 훅은 그 위의 defense in depth로 남는다. 머지 스테이지는 모든 게이트 통과 뒤 머지 직전에 머지 배우로 `gh pr review --approve` → `gh pr merge`를 한다(승인 거부는 `needs-human`이지 재시도 루프가 아니다).
  - **단일 배우 모드**(머지 토큰 없음): 보호 규칙에 승인 요건을 걸지 않는다 — 걸면 승인해 줄 두 번째 계정이 없어 다크 머지가 구조적으로 불가능해진다. 머지 권한이 에이전트 스테이지에서 도달 가능한 채로 남고, `factory doctor`가 `tokens.single-actor` WARN으로 매번 그 사실을 말한다(알려진 약화이지 틀린 설정이 아니다).
  - `doctor`는 `tokens.two-actor`/`tokens.single-actor`·`protection.two-actor`·`protection.codeowners`·`tokens.agent-is-admin`·`tokens.agent-workflow-scope` 다섯 줄로 보고한다(ADR-021 r1). 뒤의 셋은 **에이전트 배우의 토큰으로 물어야** 의미가 있으므로 CI에서만 판정한다 — 로컬에서는 **WARN "unverified until CI"**다(PASS가 아니다: 한 번도 평가되지 않은 불변식이 초록으로 보이면 그것을 부르는 CI 잡이 없다는 사실이 보이지 않는다). 부르는 곳은 sweeper의 `Doctor (merge authority)` 스텝(`node .factory/bin/doctor-ci.js`, `continue-on-error: true`)이다 — 머지 잡에서 돌리면 `GH_TOKEN`이 머지 배우의 것이라 엉뚱한 계정을 판정한다. 토큰 값은 어디에서도 읽지 않는다.
...
토큰을 쥔다) `factory/rehearsal` commit status가 폴백이다. 그 상태는 **지문 경로를 건드린 커밋들**에
붙는다 — 브랜치 head에 붙이면 무관한 머지 하나로 기록이 고아가 되어 **첫 머지부터 큐가 영구히
닫힌다**(ADR-025 r1). 쓰기는 경로마다 최신 커밋에 하나씩 올리고, 읽기는 경로마다 최근 10개 커밋을
후보로 훑어 지금의 지문을 찾으면 통과시킨다(ADR-025 r2): 그래야 쓰기와 읽기가 같은-초 동률에서
엇갈리지 않고, 해시를 바꾸지 않는 편집(주석·CHARTER 산문)이 기록을 고아로 만들지 않는다.
...
| 층 | 위치 | 에이전트 접근 | 무엇을 막나 |
|---|---|---|---|
| **L0 GitHub** | branch protection, required checks, **두 배우 토큰 스코프**(ADR-021) | 불가 | 머지, force-push, **변조된** PR의 머지(`factory/integrity`) — 보호 경로 변경은 여기서 막지 않는다(ADR-020). 두 배우 모드에서는 머지·base push가 에이전트 배우의 토큰으로 **권한상** 불가능하다(승인 1건 요구 + 자기 PR 승인 금지) |
...
- 토큰: 에이전트 잡은 전부 `FACTORY_BOT_TOKEN`을 쓴다(checkout·`GH_TOKEN` 동일) — `GITHUB_TOKEN`이 만든 라벨·push 이벤트는 다음 워크플로를 깨우지 않으므로 기본 액션 토큰으로는 스테이지 체이닝이 끊긴다. merge 잡만 `FACTORY_MERGE_TOKEN`을 쓴다(`${{ secrets.FACTORY_MERGE_TOKEN || secrets.FACTORY_BOT_TOKEN }}`, 없으면 `FACTORY_BOT_TOKEN`으로 폴백)(ADR-015 — R2). **ADR-021이 이 배선의 의미를 뒤집었다**: 예전에는 "merge 잡이 상위 토큰을 **쓸 수 있다**"였고, 지금은 "에이전트 배우는 그 일을 **할 수 없다**"이다 — 두 배우 모드에서 base 브랜치는 승인 1건을 요구하고 PR 작성자(에이전트 배우)는 자기 PR을 승인할 수 없으므로, 머지는 명령 패턴이 아니라 **권한**으로 막힌다. §4.4의 두 배우 절과 `yml-lint`의 `merge-token-scope`가 그 범위를 고정한다. 단일 배우 모드에서는 머지 보호가 예전 그대로 required checks(L0) + merge 스크립트 전용(L1, R3) + deny/훅(L2)의 합이다.
...
| 기준 | 최종 실측 (2026-09-13 16:20Z 마감, 표본 데모 5건) |
|---|---|
| 무개입 완주율 ≥80% (표본 20건) | **미달 — 머지 1 / 진입 4(프로브 제외), 엄밀한 무개입 0/4**(#8은 merge 스테이지 사람 재시도 2회). 표본이 20건에 못 미쳐 1.0 기준으로는 측정 불가이고, 측정됐다면 미달. 완주를 막은 사유는 코드 품질 0건, 전부 dogfood가 고친 결함(KTB-1~31)·계정 한도·리뷰 진동(ADR-020 "표본과 지표") |
| 손 라벨 100% 차단 | **충족 — 1/1(변형 2/2)**(이슈 #14 프로브, KTB-18); 훅 층 우회는 최종 리뷰 재리뷰 5회로 닫음 |
| R1→R2 뒤집힘 비율 | **42%(R1→R2 5/12 역할), 연속 라운드 전체 50%(14/28), approve→reject 6건** — #2·#18에서 correctness가 reject→approve→reject로 진동(O21). K=3 상한이 #2에서 라이브로 물었다(KTB-29) |
| 7일 내 revert 0건 | **0건**(머지 1건 #8 → PR #10, 창 2026-09-19 17:07Z까지 진행 중) |
...
- **Does**
  ...
  2. **선택지** (사유 코드별 기본 목록):
     ...
     - **인프라성 중단**(429·api error·timeout·cancelled·깨진 테스트 0개의 게이트 RED·라벨 유실)이고 산출물이 온전하면 `retry` — 중단 지점으로 되돌린다(`transition.js <n> --human --retry`, §3.2 사람 전용 엣지, ADR-020 KTB-32). implement가 끝나 있었으면 착지 라벨은 `awaiting-review`이고 **review만** 다시 돈다
