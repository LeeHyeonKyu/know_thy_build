...
- **토큰은 둘이고, 배우도 둘이다(ADR-021).** `FACTORY_BOT_TOKEN` = **에이전트 배우** — admin이 아닌 계정(write 협력자인 머신 유저, 나중에는 GitHub App)의 PAT이고, 체크아웃·코멘트·라벨·락 브랜치·PR 생성·`factory/*` push가 전부 이것으로 나간다. `claude -p`가 도는 스테이지가 보는 유일한 자격증명이다. `FACTORY_MERGE_TOKEN` = **머지 배우** — admin/owner PAT이고 **스크립트 전용 잡에만** 등장한다(머지 스테이지의 `Run stage` 스텝 + 그 잡의 자격증명 스크럽 스텝). 에이전트 스테이지의 `checkout` `token:`에도, 그 스텝들의 env에도 들어가지 않으며 sweeper도 받지 않는다(락 브랜치는 보호되지 않으므로 write면 충분하다). `yml-lint`의 `merge-token-scope` 규칙이 그 범위를 강제한다.
  - **두 배우 모드**(`FACTORY_MERGE_TOKEN`이 있음): `bootstrap`이 base 브랜치에 `required_pull_request_reviews = { required_approving_review_count: 1, dismiss_stale_reviews: true }`를 건다. PR을 여는 것은 언제나 에이전트 배우이고 GitHub은 자기 PR의 승인을 거부하므로(422), **그 계정의 토큰으로는 어떤 명령을 어떻게 조립해도 머지가 불가능하다** — 훅은 그 위의 defense in depth로 남는다. 머지 스테이지는 모든 게이트 통과 뒤 머지 직전에 머지 배우로 `gh pr review --approve` → `gh pr merge`를 한다(승인 거부는 `needs-human`이지 재시도 루프가 아니다).
  ...
  - `doctor`는 `tokens.two-actor`/`tokens.single-actor`·`protection.two-actor`·`tokens.agent-is-admin` 세 줄로 보고한다. 마지막 것(`gh api user` → `gh api repos/{r}/collaborators/{login}/permission`)은 CI에서만 판정한다 — 사람의 로컬 `gh`는 사람 자신이라 언제나 admin이다. 토큰 값은 어디에서도 읽지 않는다.
