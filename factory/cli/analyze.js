 ...
 */
export function parseRuns(record) {
  const runs = [];
  let cur = null;
  for (const raw of String(record ?? "").split("\n")) {
    ...
    if (s) {
      ...
      cur = {
        ...
        artifact: null, gates: null, gates_detail: [], self_gate: null,
        context: [], cost: null, agents: [], files_touched: [], notes: [], events: [],
      };
      runs.push(cur);
      continue;
    }
    ...
    if (a) { cur.artifact = a[1].trim(); continue; }

    const g = GATES_LINE.exec(line);
    if (g) { cur.gates = parseGatesLine(g[1]); continue; }

    const gd = jsonAfter(line, GATES_DETAIL_PREFIX);
    if (gd) { cur.gates_detail.push({ ...gd, bound: boundToSection(gd, cur) }); continue; }

    const cm = jsonAfter(line, CONTEXT_MANIFEST_PREFIX);
    if (cm) { cur.context.push({ ...cm, bound: boundToSection(cm, cur) }); continue; }

    const sg = jsonAfter(line, SELF_GATE_DETAIL_PREFIX);
    if (sg) { cur.self_gate = { ...sg, bound: boundToSection(sg, cur) }; continue; }

    const u = USAGE_LINE.exec(line);
    if (u) { cur.cost = parseUsage(u); continue; }

    const p = parseProgressMarker(line);
    if (p) {
      cur.agents = Array.isArray(p.agents) ? p.agents : [];
      cur.files_touched = Array.isArray(p.files_touched) ? p.files_touched : [];
      if (Number.isInteger(p.truncated_agents)) cur.truncated_agents = p.truncated_agents;
      continue;
    }
    cur.notes.push(line);
  }
...
/** 이 줄이 자기 섹션의 런을 지목하는가. 줄에 식별자가 아예 없으면 `null`(모름)이다. */
function boundToSection(line, run) {
  const runner = line?.runner == null ? null : String(line.runner);
  const runId = line?.run_id == null ? null : String(line.run_id);
  if (!runner && !runId) return null;
  if (runId && run.run_id && String(run.run_id) === runId) return true;
  if (runner && runner === run.runner) return true;
  if (runner && run.run_id && runIdOfRunner(runner) === String(run.run_id)) return true;
  return false;
}
...
export function eventsFrom(comments) {
  ...
  for (const c of comments || []) {
    ...
    if (hb?.runner) out.push({ ...base, kind: "heartbeat", stage: hb.stage ?? null, runner: hb.runner });
  }
 ...
 *
 * 이벤트는 자기보다 **앞선 마지막 런**에 붙는다(그 런이 낳은 결과이므로). 첫 런보다 이른 이벤트와
 * 기록이 아예 없는 이슈의 이벤트는 `orphan_events`로 남는다 — 버리지 않는다: 기록이 브랜치에
 * 아직 안 올라온 이슈에서도 전이 거부는 사람이 가장 보고 싶어 하는 한 줄이다.
 */
export function buildTimeline({ issue, repo = null, record = "", comments = [] } = {}) {
  const runs = parseRuns(record);
  const events = eventsFrom(comments);
  const orphan = [];

  const runMs = runs.map((r) => toMs(r.at));
  for (const ev of events) {
    if (ev.kind === "heartbeat") continue;                // 하트비트는 런의 존재 증명이지 사건이 아니다
    let idx = -1;
    for (let i = 0; i < runs.length; i += 1) {
      const rm = runMs[i];
      if (rm != null && ev.at_ms != null && rm <= ev.at_ms) idx = i;
    }
    if (idx === -1) orphan.push(ev);
    else runs[idx].events.push(ev);
  }
  ...
  return {
    issue: Number(issue), repo, runs, orphan_events: orphan,
...
export function renderTimeline(tl, classified) {
  const L = [];
  ...
  for (const [i, r] of tl.runs.entries()) {
    L.push("");
    L.push(`── ${i + 1}. ${r.stage} · ${r.at} · ${r.runner}${r.run_id && r.run_id !== r.runner ? ` (run ${r.run_id})` : ""}`);
    if (r.artifact) L.push(`   artifact: ${r.artifact}`);
    ...
    if (r.gates) {
      ...
      for (const d of r.gates_detail) {
        ...
        if (d.bound === false) bits.push("UNBOUND — this line names a different run than its section");
    ...
    if (r.self_gate) {
      const sg = r.self_gate;
      const state = sg.blocked ? (sg.harness ? "BLOCKED (harness-level — the check could not run here)" : "BLOCKED") : "ran, did not block";
      L.push(`   self-gate: ${state} · ran=${joinOr(sg.ran)} · skipped=${joinOr((sg.skipped || []).map((s) => `${s.check}(${s.reason})`))}${sg.ktb_version ? ` · ktb ${sg.ktb_version}` : ""}`);
    }

    for (const c of r.context) {
      L.push(`   context: ${c.role} — ${c.cold_read ? "cold-read" : "full-ctx"}${Number.isInteger(c.round) ? ` (round ${c.round})` : ""}, fields: ${joinOr(c.fields, "(none)")}`);
    }

    if (r.cost) {
      const cost = r.cost;
      L.push(`   cost: ${money(cost.usd)} · ${k(cost.input_tokens)} in / ${k(cost.output_tokens)} out · ${k(cost.cache_read_tokens)} cache-read · ${cost.turns ?? "?"} turns${cost.terminal_reason ? ` · ${cost.terminal_reason}` : ""}`);
    ...
    for (const line of renderEvents(r.events, "   ")) L.push(line);
  }

  if (tl.orphan_events.length) {
    L.push("");
    L.push(tl.runs.length ? "── events with no preceding stage-run" : "── events (from the issue comments)");
    for (const line of renderEvents(tl.orphan_events, "   ")) L.push(line);
  }

  L.push("");
  L.push(renderFindingsHeader(classified));
  for (const line of renderFindings(classified)) L.push(line);
  return L;
}

function renderEvents(events, pad) {
  const L = [];
  for (const ev of events) {
    if (ev.kind === "transition-refused") {
      L.push(`${pad}transition REFUSED: ${ev.from} → ${ev.to}${ev.reason ? ` — ${ev.reason}` : ""}`);
      L.push(`${pad}  (the label was moved to ${ev.moved_to})`);
    } else if (ev.kind === "transition") {
      L.push(`${pad}transition: ${ev.from} → ${ev.to} by ${ev.by}${ev.reason ? ` (${ev.reason})` : ""}`);
    } else if (ev.kind === "self-gate-retry") {
      L.push(`${pad}self-gate retry (attempt ${ev.attempt}, head ${ev.head}):`);
      for (const f of ev.findings) L.push(`${pad}  ${f.check}${f.blocking ? " [blocking]" : ""}${f.harness ? " [harness]" : ""}: ${f.detail ?? ""}`);
    } else if (ev.kind === "review") {
      const verdicts = (ev.verdicts || []).map((v) => `${v.role}=${v.verdict}`).join(", ");
      L.push(`${pad}review round ${ev.round ?? "?"}: ${verdicts || "(no verdicts)"}`);
      for (const v of ev.verdicts || []) {
        for (const mf of v.must_fix) {
          L.push(`${pad}  must_fix [${v.role}] ${mf.id ?? "?"} where=${mf.where ?? "?"} — ${mf.claim ?? ""}`);
          if (mf.evidence) L.push(`${pad}    evidence: ${mf.evidence}`);
        }
      }
    } else if (ev.kind === "handoff") {
      L.push(`${pad}handoff: ${ev.stage}${ev.summary ? ` — ${ev.summary}` : ""}`);
    } else if (ev.kind === "human-decision") {
      L.push(`${pad}human decision (${ev.skill ?? "?"}) by @${ev.author ?? "?"}: ${ev.decision ?? "?"}${ev.cause ? ` · cause=${ev.cause}` : ""}`);
      if (ev.reason) L.push(`${pad}  ${ev.reason}`);
    }
  }
  return L;
}

function renderFindingsHeader(classified) {
  if (classified.skipped) return "Findings: not classified";
  const n = classified.findings.length;
  if (!n) return "Findings: none — nothing in this issue crossed the threshold for a factory finding";
  const routed = classified.findings.filter((f) => f.disposition === "routed").length;
  return `Findings · ${n} (${routed} the retro would route) — the same classification the retro applies on merge`;
}
  ...
  if (repo) {
    try {
      ...
      if (r.code === 0 && r.stdout.trim()) {
        ...
        if (text.trim()) return { text, source: `${branch}:${dir}/${issue}.md (gh api)` };
      }
    } catch { /* 다음 경로 */ }
  }
  try {
    ...
    if (text.trim()) return { text, source: `${branch} (fetched)` };
  } catch { /* 다음 경로 */ }
  try {
    const local = join(root, dir, `${issue}.md`);
    if (existsSync(local)) {
      const text = readFileSync(local, "utf8");
      if (text.trim()) return { text, source: `${dir}/${issue}.md (local working tree)` };
    }
  } catch { /* 아래 */ }
...
export async function analyzeCommand({
  ...
  if (argv.includes("--health")) return healthReport({ root, argv, io, run, importHealth });
   ...
   * 못 알아내면 `null`을 넘긴다 — `[]`가 아니다: 봇 이름을 모르는 것은 "봇이 없다"가 아니므로
   * 그 증거 통로를 통째로 닫는 쪽이 옳다. 여기서는 회고와 **같은 판정**을 보여야 하므로, 이 명령이
   * 관대해지면 사람은 회고가 열지도 않을 상류 이슈를 기대하게 된다.
   */
  let logins = factoryLogins;
  if (logins === undefined) {
    const who = await resolveFactoryLogins({ gh: ghClient });
    logins = who.ok ? who.logins : null;
    if (!who.ok) io.err(`factory analyze: could not resolve the factory logins — ${who.reason}; human-decision attribution will be refused (the retro refuses it too)`);
  }
  ...
  for (const line of renderTimeline(tl, classified)) io.out(line);
  io.out("");
