 ...
 *
 *   1. 세션 트랜스크립트의 `Workflow` tool_result — 모델의 재타이핑을 거치지 않은 원본. 마지막 것부터.
 *   2. envelope.result 안의 ```json 펜스.
 *   3. envelope.result 안의 맨 JSON(균형 스캔).
 *
...
export function extractStageArtifact({ envelopeResult, transcriptText, validate } = {}) {
  ...
  for (let i = wf.length - 1; i >= 0; i--) {                          // 마지막 Workflow 호출이 이 스테이지의 산출물이다
    const fromFence = fencedObject(wf[i]);
    if (fromFence) candidates.push({ source: `transcript Workflow result #${i + 1} (fenced)`, obj: fromFence });
    for (const obj of balancedObjects(wf[i])) candidates.push({ source: `transcript Workflow result #${i + 1}`, obj });
  }
  ...
  if (fenced) candidates.push({ source: "result ```json fence", obj: fenced });
  for (const obj of balancedObjects(envelopeResult)) candidates.push({ source: "result bare JSON", obj });
