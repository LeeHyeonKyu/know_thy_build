...
const readRecordsDetailed = vi.fn(async () => ({ records: new Map([[45, { text: "gates-detail: mutation 71%", lines: 4 }]]) }));
vi.mock("../lib/records-branch.js", () => ({ readRecordsDetailed: (...a) => readRecordsDetailed(...a) }));
...
beforeEach(() => {
  ...
  readRecordsDetailed.mockResolvedValue({ records: new Map([[45, { text: "gates-detail: mutation 71%", lines: 4 }]]) });
...
test("test_36_human_merged_routing: the assembled routeMerged routes the issue's own comments with the run record hydrated (human-merged)", async () => {
  ...
  expect(routed.records.get(45)).toEqual({ text: "gates-detail: mutation 71%", lines: 4 });
