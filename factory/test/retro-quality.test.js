...
test("escaped_defects regression pins: own-cal R2 production-API and KTB #18 R3 finish() each register", () => {
  const commentsByIssue = new Map([
    // own-cal #3 — R1 a reviewer approves, R2 the production-API defect surfaces after that prior approve
    [3, [
      reviewHandoff(3, { round: 1, at: "2026-09-14T00:00:00Z", verdicts: [approveVerdict("architecture"), rejectVerdict("qa", "R1 finding", "q1")] }),
      reviewHandoff(3, { round: 2, at: "2026-09-14T01:00:00Z", verdicts: [rejectVerdict("correctness", "production API called from a test path", "prod-api")] }),
    ]],
    // KTB #18 — R2 a partial-panel approve, R3 full panel catches the finish() regression introduced while fixing R2
    [18, [
  ...
  const issues = [
    { number: 3, title: "own-cal", labels: ["factory:merged"], state: "closed", closedAt: "2026-09-14T02:00:00Z" },
    { number: 18, title: "ktb", labels: ["factory:merged"], state: "closed", closedAt: "2026-09-15T03:00:00Z" },
  ];
  const { stats } = harvest({ records: new Map(), issues, commentsByIssue, since: null });
  const byIssue = Object.fromEntries(stats.escaped_defects_detail.map((d) => [d.issue, d.count]));
  expect(byIssue[3]).toBeGreaterThanOrEqual(1);    // own-cal R2 production-API
  expect(byIssue[18]).toBeGreaterThanOrEqual(1);   // KTB #18 R3 finish() regression
});
...
test("accumulateStats: escaped_defects sums, revert_rate re-derives from reverts/merged, plan/implement rounds stay merge-weighted", () => {
  const w1 = { merged: 2, review_rounds_avg: 2, plan_rounds_avg: 1, implement_rounds_avg: 2, escaped_defects: 1, reverts: 0 };
  ...
  const w2 = { merged: 6, review_rounds_avg: 1, plan_rounds_avg: 1, implement_rounds_avg: 1, escaped_defects: 2, reverts: 1 };
...
test("statsTable renders the rounds-per-issue, escaped-defect and revert-rate rows plus the frozen session baseline", () => {
  ...
  // the frozen baseline (this session) and the must-not-recur set are recorded in the output
  expect(table).toContain(QUALITY_BASELINE.note);
  expect(table).toMatch(/own-cal R2 production-API/);
