...
- **사람의 머지 경로는 달라지지 않는다**: 보호 경로·역할 섹션 정책에 걸린 PR은 그대로 `needs-human`이고,
  ...
  그 전이는 리뷰 정족수·K·게이트·PR head sha·필수 체크를 **자동 머지와 같은 함수로** 다시 묻고,
  이 head가 `factory:approved`에 실제로 도달했는지(= qa 증거 게이트를 포함한 그 라벨의 요구조건이 이
  커밋에 대해 통과했는지)까지 확인합니다. 리뷰를 거치지 않은 PR을 머지했다면 거부 코멘트 한 줄과 함께
  이슈는 `needs-human`에 그대로 남습니다.
  다만 이 문(門)에서 **다시 계산하지 않는 검사가 둘** 있습니다 — ① `factory/records` run 기록과의 리뷰
  provenance 대조(자동 머지의 §(6b)), ② 그 기록의 `qa_manifest=` 다이제스트 재대조. sweep 잡에는 records
  브랜치 체크아웃이 없어서이고, KTB-48이 둘 다 덮습니다. 그 둘을 뺀 나머지는 자동 머지와 같습니다.
