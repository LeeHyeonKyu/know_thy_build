...
- `.factory/out/gates.json` (테스트 결과 원본)
- `.factory/harness.toml` — `[protected]`, `tests_are_load_bearing`, `[evidence].qa_artifacts`
- `.factory/out/qa/**` — qa 리뷰어가 남긴 증거물 (존재 여부를 당신이 확인한다)
...
5. **증거(`[evidence].qa_artifacts`) 존재 확인**: `done_when`에 사용자 가시적 항목이나 e2e 레벨이 있으면
   `.factory/out/qa/**`에 qa 리뷰어의 산출물(스크린샷·로그)이 실제로 있는가. 파일이 없는데 "확인함"이라고 적힌
   상태는 reject — 증거 없는 주장은 이 공장에서 통화가 아니다.
