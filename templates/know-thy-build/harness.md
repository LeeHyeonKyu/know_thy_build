...
gh pr list --label factory:harness --state open --json number,title,url
ls .factory/harness.toml docs/PROJECT.md 2>/dev/null
```

- doctor가 FAIL을 하나라도 보고한다 → **경로 (a)**.
- `factory:harness` PR이 열려 있다 → **경로 (c)**.
...
| `commands.*`, `commands.run.*` | `[commands]`의 명령이 이 머신에서 실제로 안 돌아감(의존성 미설치, 경로 오타) |
| `gates.*` | `[gates.required]`에 `[commands]`나 증명 세트에 없는 이름이 있음, 또는 성숙도가 그 gate를 지원하지 않음 |
| `protected.globs-match` | `[protected].factory`의 리터럴 경로가 지금 저장소에 없음(오타 또는 지워진 파일) |
| `test.smoke` | `[test].smoke`가 가리키는 파일이 실제로 없음 |
| `roles.*`, `agents.*` | `roles.toml`의 역할이 가리키는 `.claude/agents/*.md`가 없거나 §7.2 구조를 안 지킴 |
| `skills.*` | `.claude/commands/know-thy-build/*.md`가 §13.3 구조를 안 지킴 |
| `settings.*`, `hooks.*` | `.claude/settings.json`이 템플릿의 deny/hook과 안 맞음 |
| `workflows.*` | `.github/workflows/factory-*.yml`이 없거나 lint 위반 |
| `github.*` | secret 미설정, 라벨 미부트스트랩, branch protection 미설정(대부분 WARN — `factory bootstrap` 재실행으로 해결) |
...
`factory:harness` PR은 일반 파이프라인(plan→implement→review)을 거쳐 열린 것이지만, `harness.toml`(또는 다른 `[protected]` 경로)을 건드리므로 integrity가 자동 머지를 막는다 — 사람이 GitHub UI에서 머지해야 한다. 이 스킬은 그 diff를 사람이 5분 안에 판단할 수 있는 요약으로 바꾼다.

### Step 1: diff를 세 가지로 요약
...
2. **비용** — CI 로그나 PR의 run 기록에서 소요 시간·토큰을 찾는다(`docs/factory/runs/<issue>.md`).
...
### Step 2: 한 화면 요약 + GitHub 링크 제시

```
PR #<n> · factory:harness · <title>
────────────────────────────────────────
gate 추가: [gates.required]에 "coverage" 추가, [harness].maturity M0 → M1
비용: run #<issue> — 42분, 380k 토큰
스모크: factory/gates ✓ · factory/review ✓ · factory/integrity ✓ (모두 GREEN)

머지는 사람이 GitHub에서: https://github.com/<owner>/<repo>/pull/<n>
```
