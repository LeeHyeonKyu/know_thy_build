...
| `tokens.two-actor` / `tokens.single-actor`, `protection.two-actor`, `protection.codeowners`, `tokens.agent-is-admin`, `tokens.agent-workflow-scope` | 머지 권한이 어디에 있는가(ADR-021). `tokens.single-actor` WARN = `FACTORY_MERGE_TOKEN`이 없어 머지 권한이 에이전트 스테이지에서 도달 가능하다(훅이 유일한 층). `protection.two-actor` FAIL = 머지 토큰은 있는데 base 브랜치에 승인 요건이 없거나 `require_code_owner_reviews`가 빠졌다 → `factory bootstrap` 재실행. `protection.codeowners` FAIL = `.github/CODEOWNERS`가 없거나 `*`의 소유자가 없거나 **에이전트 로그인이 들어 있다**(그러면 에이전트가 스스로 승인해 머지할 수 있다, r1 MF-1). `tokens.agent-is-admin` FAIL = 봇 계정이 admin/maintain이라 두 배우 모드가 아무것도 강제하지 못한다 → write 협력자로 낮춘다. `tokens.agent-workflow-scope` FAIL = 봇 PAT에 classic `workflow` 스코프가 붙어 있다 → 에이전트가 워크플로 한 장으로 머지 토큰을 읽어 갈 수 있다, `repo`만으로 재발급(r1 MF-2). 뒤의 셋은 **CI에서만** 판정한다(sweeper의 `Doctor (merge authority)` 스텝) — 로컬에서는 WARN "unverified until CI"다 |
